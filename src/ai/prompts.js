...
- 個人の日常投稿は除外
- **まとめ記事は除外**: 複数のイベントを一覧・紹介する投稿（「今週のイベント」「○月のイベントまとめ」「おでかけ情報」等）は単一イベントの告知ではないため除外。1つの特定イベントの告知のみを対象とする。

## 出力（JSON）
{
  "is_event": true/false,
  "confidence": "high"/"medium"/"low",
  "event_type": "告知"/"レポート"/"まとめ"/"その他",
  "reason": "判定理由"
}
`;
...
- 時間情報がない場合は空文字
- exhibitorsは説明文中に店舗名・出展者名・ブース名（固有名詞）が含まれている場合のみ抽出。なければ空配列[]
- exhibitors.nameは必ず固有名詞（店舗名・屋号・団体名・個人名）。「お菓子」「パン」「ワークショップ」等のカテゴリ名や一般名詞は不可。名前が不明な出展者はスキップ
- exhibitors.menuは具体的なメニュー名と価格がわかる場合のみ。不明なら空配列[]
`;
