...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/outreach-$(date +%Y%m%d-%H%M%S).log"

echo "=== Outreach started at $(date) ===" >> "$LOG_FILE"
node src/send-outreach-auto.mjs >> "$LOG_FILE" 2>&1
EXIT_CODE=$?
echo "=== Outreach finished at $(date) with exit code $EXIT_CODE ===" >> "$LOG_FILE"

# 投稿後のstateをDriveに戻す
cp data/state.json "$SYNC/state.json" 2>/dev/null

# Keep only last 30 log files
ls -t "$LOG_DIR"/outreach-*.log 2>/dev/null | tail -n +31 | xargs rm -f 2>/dev/null
